...
int main(int argc, char const *argv[]){
	...
	for(i=0;i<num_rounds;i++)
	{
		...
		#pragma omp parallel num_threads(num_cashiers) shared(lck) private(tid,j,k,valid,qtd_itens,itens)
		{
			...
			sort(itens, itens + qtd_itens);//ordena vetor de itens de cada thread para que o acesso ao recurso seja sequencial e controlado implicitamente pelo historico de qual thread ja acessou o recurso primeiro
